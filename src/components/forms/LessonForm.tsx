        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select className="ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("day")} defaultValue={data?.day}>
            {["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY"].map((d) => (
              <option value={d} key={d}>{d.charAt(0) + d.slice(1).toLowerCase()}</option>
            ))}
          </select>
          {errors.day?.message && <p className="text-xs text-red-400">{errors.day.message.toString()}</p>}
        </div>

        <InputField label="Start Time" name="startTime" type="datetime-local" defaultValue={data?.startTime?.toISOString().slice(0,16)} register={register} error={errors.startTime} />
        <InputField label="End Time" name="endTime" type="datetime-local" defaultValue={data?.endTime?.toISOString().slice(0,16)} register={register} error={errors.endTime} />
