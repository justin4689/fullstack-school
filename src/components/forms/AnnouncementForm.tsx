...
const AnnouncementForm = ({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AnnouncementSchema>({ resolver: zodResolver(announcementSchema) });
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label="Date" name="date" type="datetime-local" defaultValue={data?.date?.toISOString().slice(0,16)} register={register} error={errors.date} />
        {data && (
