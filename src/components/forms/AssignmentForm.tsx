  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label="Start Date" name="startDate" type="datetime-local" defaultValue={data?.startDate?.toISOString().slice(0,16)} register={register} error={errors.startDate} />
        <InputField label="Due Date" name="dueDate" type="datetime-local" defaultValue={data?.dueDate?.toISOString().slice(0,16)} register={register} error={errors.dueDate} />
        {data && (
